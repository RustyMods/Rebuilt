...
namespace Rebuilt
{
    ...
    public class RebuiltPlugin : BaseUnityPlugin
    {
        ...
        public void Awake()
        {
            ...
            _requireResources = config("2 - Settings", "Require Resources", Toggle.On, "If on, resources drop on destroy, and requires resources to rebuild");
            _transparency = config("2 - Settings", "Transparency", 0.5f, new ConfigDescription("Set level of transparency of ghost piece", new AcceptableValueRange<float>(0f, 1f)));
            ...
            _ghostSupports = config("2 - Settings", "Supports", Toggle.Off, "If on, ghost pieces supports");
            ...
            _blacklist = config("3 - Blacklist", "Prefab Names", "", new ConfigDescription(
                "List of prefab names that are excluded from ghosting", null, new ConfigurationManagerAttributes()
                {
                    Category = "3 - Blacklist",
                    CustomDrawer = SerializedNameList.Draw
                }));
            _hasCreator = config("2 - Settings", "Require Creator", Toggle.On, "If on, pieces require a creator to become ghosts");
